docker build -t snehaprem/multi-client:latest -f ./client/Dockerfile ./client
docker build -t snehaprem/multi-server:latest -f ./server/Dockerfile ./server
docker build -t snehaprem/multi-worker:latest -f ./worker/Dockerfile ./worker
docker build -t snehaprem/multi-client:$SHA -f ./client/Dockerfile ./client
docker build -t snehaprem/multi-server:$SHA -f ./server/Dockerfile ./server
docker build -t snehaprem/multi-worker:$SHA -f ./worker/Dockerfile ./worker
